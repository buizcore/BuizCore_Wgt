...
abstract class WgtInput extends WgtAbstract
{
    ...
    /**
     * Flag ob die Box einfach mit display none ausgeblendet werden soll
     * 
     * @var boolean
     */
    ...
    /**
     * Flag das element als Input gerendet werden soll
     * @var boolean
     */
    ...
    /**
     * Rendermodus wenn abweichend vom Standard gerendert werden soll
     * Zb Cluster / Matrix Modus für Checkboxes
     * @var string
     */
    ...
    public function setReadOnly($readOnly = true)
    {
        ...
        if ($readOnly) {
            ...
            /*
             * if (isset($this->attributes['disabled'])) unset($this->attributes['disabled']);
             */
    ...
    {
        if ($this->docu) {
            return '<span class="wcm wcm_ui_dropform help" id="wgt-input-help-'.$id.'" ><i class="fa fa-info-circle" ></i><var>{"size":"small","button":""}</var></span>'.'<div class="wgt-input-help-'.$id.' hidden" ><div class="wgt-panel title" ><h2>Help</h2></div><div class="wgt-space" >'.$this->docu.'</div></div>';
        }
...
HTML;
        ...
        $html = <<<HTML
    <div class="wgt-box input  has-clearfix {$boxClasses}" id="wgt-box-{$id}" {$styleHidden} >
      {$this->texts->topBox}
...
HTML;
    ...
    public function buildAjaxArea()
    {
        ...
        if ($this->serializeElement) {
            $html = '<htmlArea selector="input#'.$this->attributes['id'].'" action="thml" ><![CDATA['.$this->element().']]></htmlArea>'.NL;
        } else {
    ...
    protected function asmAttributes($attributes = array(), $setType = true)
    {
        ...
        if (!$attributes) {
            ...
            if (isset($this->attributes['class'])) {
                $safeClass = $this->attributes['class'];
            }
            
            $oldAttr = $attributes;
            $attributes = array_merge($this->attributes, $attributes);
            
            if (isset($oldAttr['class'])) {
                $attributes['class'] = $safeClass.' '.$attributes['class'];
            }
        }
