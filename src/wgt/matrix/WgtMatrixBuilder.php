...
HTML;
    ...
    <button class="wgt-button" onclick="\$R.get('{$this->addURL}');" ><i class="fa fa-plus-circle" ></i> Create</button>
    &nbsp;|&nbsp;&nbsp;
     */
...
<var id="{$this->id}-table-cfg-grid" >{
  ...
  <div class="wgt-panel" >
    ...
    <label>Show as:</label> <select
      ...
      class="fparam-{$this->searchForm} medium"
        >{$codeVariants}</select>
    &nbsp;&nbsp; <button class="wgt-button" onclick="\$R.form('{$this->searchForm}');"  ><i class="fa fa-refresh" ></i> Refresh</button>
  </div>

  <table class="wgt-grid wcm wcm_widget_grid hide-head wgt-matrix" id="{$this->id}-table" style="table-layout: fixed;" >
    <thead>
      <tr>
{$mHead}
      </tr>
