...
class WgtMenuBuilderPaging
{
  ...
  public function pagingMenu($linkTarget, $dataSize, $stepSize, $anzMenuNumbers, $ajax = true)
  {
    ...
    for ($nam = $startPos; $nam < $endPos; ++ $nam) {
      ...
      if ($ajax) {
        $urlClass = ($nam == $activPos) ? 'class="wgt-active wcm wcm_req_page ' . $this->element->searchForm . '"' : 'class="wcm wcm_req_page ' . $this->element->searchForm . '"';
      } else {
        $urlClass = ($nam == $activPos) ? 'class="wgt-active"' : '';
      }
