...
/*////////////////////////////////////////////////////////////////////////////*/
  ...
  protected function buildButtons($buttons = null)
  {
    ...
    foreach ($buttons as $button) {
      ...
      $icon = Wgt::icon($button[Wgt::BUTTON_ICON] ,'xsmall', $button[Wgt::BUTTON_LABEL]);
      ...
      if (is_array($button)) {
          ...
          if(is_array($button[Wgt::BUTTON_LABEL])){
            ...
            if(count($button[Wgt::BUTTON_LABEL]) == 3 ){
                $i18nText = $i18n->l(
                    $button[Wgt::BUTTON_LABEL][0],
                    $button[Wgt::BUTTON_LABEL][1],
                    $button[Wgt::BUTTON_LABEL][2]
                );
            } else {
                $i18nText = $i18n->l(
                    $button[Wgt::BUTTON_LABEL][0],
      ...
      if (is_object($button)) {
        ...
        $html .= '<div class="wgt-panel-control" >'.$button->render().'</div>'.NL;

      } elseif (is_string($button)) {
        
        $html .= '<div class="wgt-panel-control" >'.$button.'</div>'.NL;
        
      } elseif ($button[Wgt::BUTTON_TYPE] == Wgt::ACTION_URL) {

        $html .= '<div class="wgt-panel-control" style="padding-top:5px;">'.Wgt::urlTag(
        ...
        $html .= '<div class="wgt-panel-control" >'.Wgt::urlTag(
          $button[Wgt::BUTTON_ACTION],
          $icon.' '.$i18nText,
          array(
            'class'=> $button[Wgt::BUTTON_PROP],
        ...
        $html .= '<div class="wgt-panel-control" ><button '
          .' onclick="$R.get(\''.$url.'\');return false;" '
          .' class="'.$button[Wgt::BUTTON_PROP].'" '
          .' title="'.$i18nText.'" >'
          .$icon.' '
          .$i18nText.'</button></div>'.NL; // ' '.$button[Wgt::BUTTON_LABEL].

      } elseif ($button[Wgt::BUTTON_TYPE] == Wgt::ACTION_JS) {

        $html .= '<div class="wgt-panel-control" >'
            .'<button onclick="'.$button[Wgt::BUTTON_ACTION].';return false;" '
            .' class="'.$button[Wgt::BUTTON_PROP].'" title="'.$i18nText.'" >'
            .$icon
            .' '.$i18nText.'</button></div>'.NL; // ' '.$button[Wgt::BUTTON_LABEL].

      } else {

        $html .= '<div class="wgt-panel-control" ><button onclick="'.$button[Wgt::BUTTON_ACTION].';return false;" '
          .' class="'.$button[Wgt::BUTTON_PROP].'" '
          .' title="'.$i18nText.'" >'
          .$icon
          .' '.$i18nText.'</button></div>'.NL; // ' '.$button[Wgt::BUTTON_LABEL].
