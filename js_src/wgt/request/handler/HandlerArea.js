...
(function($S,$R){
  ...
  var actions ={
     ...
     'removeClass':function(selector,text){$S(selector).removeClass( text );},  
  ...
  $R.getHandler().addElementHandler( 'htmlArea', function( htmlAreas ){
    ...
    if( htmlAreas.get().length > 0 ) {
      ...
      htmlAreas.each(function() {
        ...
        var selectorElse = $S.trim( htmlArea.attr('select_else') );
        if( ''==selectorElse )
            selectorElse = areaId;
          ...
          if( checkSelector ){
            ...
            if( checkNot ){
              ...
              if( $S(checkSelector).length==0 ){
                  ...
                  console.log("found no "+selectorElse+" execute "+action+" to "+areaId  );
                actions[action](areaId, htmlArea.text());
              }
              ///TODO need some warning here?
              else if( actionElse && actions[actionElse] !== undefined ){

                  console.log("actionElse not "+actionElse + ' selector '+selectorElse );
