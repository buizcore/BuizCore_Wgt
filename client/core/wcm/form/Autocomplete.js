...
$R.addAction( 'ui_autocomplete', function( jNode ){
  ...
  if( tmpProps.type && 'entity' == tmpProps.type ) {
    ...
    props.select = function( event, ui ){
      ...
      if( ui.item ) {
        ...
        if( tmpProps.formid ) {
          ...
          console.log("Send to "+tmpProps.send_to);
          var dataBody = '';
    ...
    */
    ...
    if( !tmpProps.formid ) {
      $D.error(
        'Sorry, the programmer forgot to give an address for the autocomplete service.'
        +' This is a bug. Please report to the author of tool.'
      );
      return;
    }
