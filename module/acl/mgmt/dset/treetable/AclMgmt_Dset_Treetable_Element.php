  ...
  public function buildHtml()
  {
    ...
    if ($this->insertMode) {
      ...
      $this->html .= '<table id="'.$this->id
        .'-table" class="wgt-grid wcm wcm_widget_grid wcm_ui_treetable hide-head full" >'.NL;
 ...
 /**
  ...
  public function buildTbody()
  {
    ...
    foreach ($this->data as $row) {
      ...
      $body .= '<td valign="top" ><i class="fa fa-group" ></i> '.$row['role_group_name'].'</td>'.NL;
  ...
  public function buildUserNode($groupId, $parentPos)
  {
    ...
    foreach ($childs as $userId => $row) {
      ...
      $body .= '<td valign="top" class="ind1" ><i class="fa fa-user" ></i> '.$row['user'].'</td>'.NL;
      $body .= '<td valign="top" >';

      if (!is_null($row['group_users_vid'])) {
        $body .= '<em><i class="fa fa-file-alt " ></i> Dataset</em>';
      } elseif (isset($row['group_users_id_area']) && !is_null($row['group_users_id_area'])) {
        $body .= '<em><i class="fa fa-list-alt" ></i> All Projects</em>';
      } else {
  ...
  public function buildAjaxEntry()
  {
    ...
    foreach ($this->data as $key => $row) {
      ...
      if (!is_null($row['group_users_vid'])) {
        ...
        $body .= '<em><i class="fa fa-list-alt" ></i> Table</em>';
      } else {
        $body .= '<em>'.$this->icon('relation/global.png','Global').' Global</em>';
      }
