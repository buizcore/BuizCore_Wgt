...
/*////////////////////////////////////////////////////////////////////////////*/
  ...
  */
  public function loadUrl()
  {
    ...
    $this->id = 'wgt-treetable-'.$this->domainNode->domainName.'-acl-tuser';
    ...
    $this->actions['dset'] = array('dset_mask', 'sep',  'delete');
 ...
 /**
  ...
  <tr class="wcm wcm_ui_highlight row{$num} wgt-border-top flag_partial" id="{$rowid}"  >
    ...
    <td valign="top" class="ind1" ><span
        ...
        wgt_eid="{$userId}" >{$icons['closed']}</span> <i class="fa fa-user" ></i>
      <a
        class="wcm wcm_req_ajax"
        href="modal.php?c=Buiz.ContactForm.formUser&amp;user_id={$row['role_user_rowid']}&amp;d_src={$this->domainNode->domainName}" >
  ...
  public function renderDsetBlock($userId, $context)
  {
    ...
    foreach ($this->dataEntity as $row) {
      ...
      if ($this->enableNav) {
        $navigation = $this->rowMenu
        (
          '0&dset_id='.$dsetId.'&user_id='.$userId,
          $row,
          null,
          null,
          'dset'
      ...
      <tr class="wcm wcm_ui_highlight row{$num} {$pRowid}" id="{$rowid}" >
        ...
        <td valign="top" class="ind1" >&nbsp;&nbsp;
          <span
            ...
            wgt_param="&amp;user={$userId}" wgt_eid="{$dsetId}" >{$icons['closed']}</span>
...
HTML;
  ...
  public function renderGroupBlock($userId, $dsetId, $context)
  {
    ...
    foreach ($this->dataGroup as  $row) {
      ...
      $rowid = $this->id.'_row_'.$userId.'_'.$dsetId.'_'.$groupId;
  ...
  public function buildAjaxUserNode($groupId)
  {
    ...
    foreach ($childs as $key => $row) {
      ...
      if (isset($row['id'])) {
        ...
        $body .= '<td valign="top" class="pos" >1</td>'.NL;
        $body .= '<td valign="top" class="ind1" ><i class="fa fa-user" ></i> '.$row['name'].' (partial)</td>'.NL;
        ...
        $body .= '<td valign="top" class="pos" >1</td>'.NL;
        $body .= '<td valign="top" ><i class="fa fa-user" ></i> '.$row['user'].'</td>'.NL;
        $body .= '<td valign="top" >'
          .'<input
            type="text"
            class="'.$this->editForm.' wcm wcm_ui_date show small"
...
/*////////////////////////////////////////////////////////////////////////////*/
  ...
  public function buildTableFooter()
  {
    ...
    // <li><a><i class="fa fa-times" ></i> Delete Selection</a></li>
  ...
  <div class="wgt-dropdownbox" id="{$this->id}-list-action-menu" >
    <ul>
      <li><a
        ...
        href="ajax.php?c=Acl.Mgmt_Qfdu.dropAllAssignments&amp;dkey={$this->domainNode->domainName}" ><i class="fa fa-eraser" ></i> Delete all</a></li>
      <li><a
        target="_document"
        href="document.php?c=Acl.Mgmt_Qfdu_User.export&amp;dkey={$this->domainNode->domainName}" ><i class="fa fa-file-text-alt" ></i> Export</a></li>
    </ul>
   </div>
