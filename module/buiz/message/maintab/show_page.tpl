...
$sForm = new WgtFormBuilderElements(
  $this,
  'ajax.php?c=Buiz.Message.saveMessage&objid='.$VAR->msgNode->msg_id,
  'msg-show-save-'.$VAR->msgNode->msg_id,
  'put'
);


$selectMessageTaskStatus = new BuizMessageTaskStatus_Selectbox($this);
$selectMessageTaskStatus->addAttributes(array(
  'name' => 'task[status]',
  'class' => $sForm->dKey,
));

$apointCategory = $sForm->loadQuery( 'BuizAppointmentCategory_Selectbox' );
  ...
  <div id="wgt-acc-show-msg-<?php echo $VAR->msgNode->msg_id; ?>"  >
    ...
    <div class="ac_body np"  >
      ...
      <ul class="wgt-list wgt-space" >
        ...
        <li><label>Confidential:</label> <span><?php echo EBuizConfidential::label($VAR->msgNode->confidential); ?></span></li>
      </ul>
      ...
      <div id="wgt-mox-show-msg-task-<?php echo $VAR->msgNode->msg_id; ?>" >
        ...
        <ul class="wgt-list kv wgt-space" >
          <li><label>Status:</label> <span><?php $sForm->selectboxByKey(null,
          		'task[status]',
          		'BuizMessageTaskStatus_Selectbox',
      ...
      <div id="wgt-box-show-msg-appoint-<?php echo $VAR->msgNode->msg_id; ?>" >
        ...
        <ul class="wgt-list kv wgt-space" >
          <li><label>Category:</label> <span><?php $sForm->selectboxByKey(null,
          		...
          		'BuizAppointmentCategory_Selectbox',
    ...
    <div class="ac_body np"  >
      <div id="wgt-kvl-msg-checklist-<?php echo $VAR->msgNode->msg_id; ?>" class="wcm wcm_widget_kvlist" >
        <var id="wgt-kvl-msg-checklist-<?php echo $VAR->msgNode->msg_id; ?>-cfg-kvlist" >{
          ...
          "edit_able":true,
          "srv_delete":"ajax.php?c=Buiz.Message_Checklist.delete&delid="
        }</var>

        <form
          id="wgt-form-save-kvl-msg-checklist-<?php echo $VAR->msgNode->msg_id; ?>"
          method="put"
          action="ajax.php?c=Buiz.Message_Checklist.save&msg=<?php echo $VAR->msgNode->msg_id; ?>" ></form>
    ...
    <div class="ac_body np"  >
      <button class="wgt-button" ><i class="fa fa-plus-circle" ></i> Share</button>
...
<div
  ...
  <div class="wgt-panel" >
    ...
    <div class="right" ><button
      class="wgt-button wgac_add_attachment" ><i class="fa fa-plus-circle" ></i></button></div>
  </div>
    ...
    <ul
     ...
     <?php foreach( $VAR->attachments as $attach ){  ?>
        <li id="wgt-entry-msg-attach-<?php echo $attach['attach_id']; ?>" ><a
          target="attach"
          href="file.php?f=buiz_file-name-<?php echo $attach['file_id']; ?>&n=<?php echo base64_encode($attach['file_name'])  ?>"
          ><?php echo $attach['file_name']; ?></a><a
          class="wcm wcm_req_del"
          title="Please confirm you want to delete this Attachment"
          href="ajax.php?c=Buiz.Message_Attachment.delete&delid=<?php echo $attach['attach_id']; ?>"  ><i class="fa fa-times" ></i></a></li>
      <?php } ?>
    </ul>
  </div>
  ...
  <div class="wgt-panel" >
    ...
    <div class="right" ><button
      ...
      id="wgt-button-message-addref-<?php echo $VAR->msgNode->msg_id; ?>" ><i class="fa fa-plus-circle" ></i></button></div>
  </div>
  <div class="content hr15_25x wgt-space" >
    <ul
      ...
      <?php foreach( $VAR->refs as $ref ){  ?>
        <li id="wgt-entry-msg-ref-<?php echo $ref['link_id']; ?>" ><a
          ...
          href="ajax.php?c=Buiz.Message.delRef&delid=<?php echo $ref['link_id']; ?>"  ><i class="fa fa-times" ></i></a></li>
      <?php } ?>
    </ul>
  </div>
</div>
...
$S('#wgt-button-message-addref-<?php echo $VAR->msgNode->msg_id; ?>').on('connect',function(event,id){
  $R.put('ajax.php?c=Buiz.Message.addRef&msg=<?php echo $VAR->msgNode->msg_id; ?>&ref='+id);
});

self.getObject().find(".wgac_add_attachment").click( function(){
  $R.get( 'modal.php?c=Buiz.Message_Attachment.formNew&msg=<?php echo $VAR->msgNode->msg_id; ?>' );
});

self.getObject().find(".wgac_add_reference").click( function(){
  $R.get( 'modal.php?c=Buiz.DataConnector.selection&con=Buiz.Message.connectDset&cbe=wgt-button-message-addref-<?php echo $VAR->msgNode->msg_id; ?>&dset=<?php echo $VAR->msgNode->msg_id; ?>' );
});
