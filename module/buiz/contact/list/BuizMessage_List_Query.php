 ...
 * @author Dominik Bonsch <d.bonsch@buizcore.com>
 */
class BuizMessage_List_Query extends LibSqlQuery
{
 ...
 /**
  ...
  public function setCalcQuery($criteria, $params)
  {
    ...
    if ($params->loadFullSize)
      $this->calcQuery = $criteria->count('count(buiz_contact.rowid) as '.Db::Q_SIZE);
...
/*////////////////////////////////////////////////////////////////////////////*/
 ...
 /**
  ...
  public function fetch($condition = null, $params = null)
  {
    ...
    if ($params->loadFullSize)
      $this->calcQuery = $criteria->count('count(buiz_contact.'.Db::PK.') as '.Db::Q_SIZE);
 ...
 /**
  ...
  public function setCols($criteria)
  {
    ...
    $cols = array(
      'buiz_contact.rowid as "contact_id"',
    );
  ...
  public function setTables($criteria)
  {
    ...
    $criteria->from('buiz_contact as contact','buiz_contact');

    // der sender
    /*
    $criteria->joinOn(
      'buiz_contact', 'id_sender',
    ...
    if ($this->condition) {
      ...
      if (is_string($this->condition)) {
        ...
        if (ctype_digit($this->condition)) {
          $criteria->where('buiz_contact.rowid = '.$this->condition);
        } else {
    ...
    if ($condition) {
      ...
      if (is_string($condition)) {
        if (ctype_digit($condition)) {
          $criteria->where('buiz_contact.rowid = '.$condition);
        } else {
 ...
 /**
  ...
  public function checkConditions($criteria, array $condition)
  {
    ...
    if (isset($condition['free']) && trim($condition['free']) != '') {
       ...
       if (ctype_digit($condition['free'])) {
          ...
          $criteria->where('( buiz_contact.rowid = \''.$part.'\' )');
            ...
            foreach ($parts as $part) {
              ...
              if ('@' == $safePart[0]) {
                ...
                ('(
                  ...
                  UPPER(buiz_contact.title) like UPPER(\'%'.$safePart.'%\')
                    OR UPPER(buiz_contact.message) like UPPER(\'%'.$safePart.'%\')
                )');
              }
    ...
    // search conditions for  buiz_contact
    if (isset($condition['buiz_contact'])) {
      $whereCond = $condition['buiz_contact'];

      if (isset($whereCond['title']) && trim($whereCond['title']) != '')
        $criteria->where(' buiz_contact.title = \''.$whereCond['title'].'\' ');

      // append meta information
      if (isset($whereCond['m_role_created']) && trim($whereCond['m_role_created']) != '')
        $criteria->where(' buiz_contact.m_role_created = '.$whereCond['m_role_created'].' ');

      if (isset($whereCond['m_role_changed']) && trim($whereCond['m_role_changed']) != '')
        $criteria->where(' buiz_contact.m_role_changed = '.$whereCond['m_role_changed'].' ');

      if (isset($whereCond['m_time_created_before']) && trim($whereCond['m_time_created_before']) != '')
        $criteria->where(' buiz_contact.m_time_created <= \''.$whereCond['m_time_created_before'].'\' ');

      if (isset($whereCond['m_time_created_after']) && trim($whereCond['m_time_created_after']) != '')
        $criteria->where(' buiz_contact.m_time_created >= \''.$whereCond['m_time_created_after'].'\' ');

      if (isset($whereCond['m_time_changed_before']) && trim($whereCond['m_time_changed_before']) != '')
        $criteria->where(' buiz_contact.m_time_changed <= \''.$whereCond['m_time_changed_before'].'\' ');

      if (isset($whereCond['m_time_changed_after']) && trim($whereCond['m_time_changed_after']) != '')
        $criteria->where(' buiz_contact.m_time_changed >= \''.$whereCond['m_time_changed_after'].'\' ');

      if (isset($whereCond['m_rowid']) && trim($whereCond['m_rowid']) != '')
        $criteria->where(' buiz_contact.rowid >= \''.$whereCond['m_rowid'].'\' ');

      if (isset($whereCond['m_uuid']) && trim($whereCond['m_uuid']) != '')
        $criteria->where(' buiz_contact.m_uuid >= \''.$whereCond['m_uuid'].'\' ');

    }//end if (isset ($condition['buiz_contact']))
  ...
  public function checkCharBegin($criteria, $params)
  {
    ...
    if ($params->begin) {
      ...
      if ('?' == $params->begin) {
        $criteria->where("buiz_contact.title ~* '^[^a-zA-Z]'");
      } else {
        $criteria->where("upper(substr(buiz_contact.title,1,1)) = '".strtoupper($params->begin)."'");
      }
  ...
  public function checkLimitAndOrder($criteria, $condition, $params)
  {
    ...
    if ($condition['order']) {
      ...
      $criteria->orderBy('buiz_contact.m_time_created desc');
  ...
  public function injectOrder($criteria, $params)
  {
    ...
    $criteria->orderBy('buiz_contact.m_time_created asc');
