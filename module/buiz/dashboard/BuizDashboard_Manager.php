 ...
 * @author Dominik Bonsch <d.bonsch@buizcore.com>
 */
class BuizDashboard_Manager extends Manager
{
...
WHERE
  (NOT uss.visited = '2' OR uss.visited is null)
  	AND ann.type = {$type}
  	AND (ann.date_start <= '{$now}' OR ann.date_start is null)
  	AND (ann.date_end >= '{$now}' OR ann.date_end is null)
...
SQL;
    ...
    /*
( UPPER(chan.access_key) = UPPER('wbf_global') OR ann.id_channel is null )
  	AND 
     */
