 ...
 * @author Dominik Bonsch <d.bonsch@buizcore.com>
 */
class BuizProtocol_Overlay_Query extends LibSqlQuery
{
 ...
 /**
  ...
  public function fetch($condition = null, $params = null)
  {
    ...
    $criteria = new LibSqlCriteria('buiz-protocol-overlay', $db);
    ...
    if ($params->loadFullSize)
      $this->calcQuery = $criteria->count('count(DISTINCT buiz_protocol_message.rowid) as '.Db::Q_SIZE);
 ...
 /**
  ...
  public function fetchInAcls(array $inKeys, $params = null)
  {
    ...
    $criteria->where
    (
      " buiz_protocol_message.rowid  IN(". implode(', ', array_keys($inKeys)) .")"
    );
 ...
 /**
  ...
  public function setCols($criteria)
  {
    ...
    $cols = array
    (
      'DISTINCT buiz_protocol_message.rowid as "log_rowid"',
      'buiz_protocol_message.message as "log_content"',
      'buiz_role_user.name as "user_name"',
      'core_person.rowid as "person_id"',
      'core_person.firstname as "person_firstname"',
      'core_person.lastname as "person_lastname"',
      'buiz_protocol_message.m_time_created'
    );
    ...
    if ($this->condition) {
      ...
      if (is_string($this->condition)) {
        ...
        if (ctype_digit($this->condition)) {
          $criteria->where('buiz_protocol_message.rowid = '.$this->condition);
        } else {
    ...
    if ($condition) {
      ...
      if (is_string($condition)) {
        if (ctype_digit($condition)) {
          $criteria->where('buiz_protocol_message.rowid = '.$condition);
        } else {
 ...
 /**
  ...
  public function checkConditions($criteria, array $condition)
  {
      ...
      if (isset($condition['ids']) && !empty($condition['ids'])) {
        $criteria->where
        (
          'buiz_protocol_message.rowid IN('. implode(', ', $condition['ids']) .') '
        );
      }

      if (isset($condition['vid'])) {
        $criteria->where('buiz_protocol_message.vid = '.$condition['vid'].'  ');
      }

      if (isset($condition['id_entity'])) {
        $criteria->where('buiz_protocol_message.id_vid_entity = '.$condition['id_entity'].'  ');
      }

      if (isset($condition['id_mask'])) {
        $criteria->where('buiz_protocol_message.id_mask = '.$condition['id_mask'].'  ');
      }
      ...
      if (isset($condition['free']) && trim($condition['free']) != ''  ) {
         ...
         )
         {
            ...
            $criteria->where
            (
              '( buiz_protocol_message.rowid = \''.$part.'\')'
            );
         } else {
      ...
      if (isset($condition['buiz_protocol_message'])) {
        $whereCond = $condition['buiz_protocol_message'];
        ...
        */
        ...
        if (isset($whereCond['m_role_created' ]) && trim($whereCond['m_role_created']) != ''  )
          $criteria->where(' buiz_protocol_message.m_role_created = '.$whereCond['m_role_created'].' ');

        if (isset($whereCond['m_time_created_before']) && trim($whereCond['m_time_created_before']) != ''  )
          $criteria->where(' buiz_protocol_message.m_time_created <= \''.$whereCond['m_time_created_before'].'\' ');

        if (isset($whereCond['m_time_created_after']) && trim($whereCond['m_time_created_after']) != ''  )
          $criteria->where(' buiz_protocol_message.m_time_created >= \''.$whereCond['m_time_created_after'].'\' ');

      }//end if (isset ($condition['buiz_protocol_message']))
  ...
  public function checkLimitAndOrder($criteria, $params  )
  {
    ...
    /**/
    $criteria->orderBy('buiz_protocol_message.m_time_created');
    $criteria->selectAlso('buiz_protocol_message.m_time_created as "buiz_protocol_message-m_time_created-order"');
  ...
  public function injectAclOrder($criteria, $envelop, $params  )
  {
    ...
    $envelop->groupBy('inner_acl."buiz_protocol_message-m_time_created-order"');
    $envelop->selectAlso('inner_acl."buiz_protocol_message-m_time_created-order"');
    $envelop->orderBy('inner_acl."buiz_protocol_message-m_time_created-order"');
